# =================== ARCHIVO: venezuela.py (CÓDIGO COMPLETO CORREGIDO) ===================
"""
...
def diagnosticar_archivo_venezuela(archivo):
    """Función específica para diagnosticar archivos de Venezuela"""
    print(f"\n🇻🇪 DIAGNÓSTICO ESPECÍFICO VENEZUELA")
    print("=" * 50)
...
def procesar_datos_venezuela_especifico(df):
    """Procesamiento específico de datos de Venezuela con ajuste de CAPEX según prioridad"""
    print("🇻🇪 Aplicando procesamiento específico de Venezuela...")
    ...
    if col_moneda:
        ...
        if vef_count > 0:
            print(f"🔄 Convirtiendo {vef_count} registros de VEF → VES")
...
def procesar_venezuela(archivo_reporte_pago, archivo_reporte_absoluto=None):
    ...
    print("🇻🇪 PROCESANDO CONSOLIDADO CAPEX VENEZUELA + REPORTE ABSOLUTO")
    print("=" * 70)
    ...
    try:
        ...
        print(f"✅ Tasa seleccionada: {tasa_dolar:.4f} VES/USD (fecha: {fecha_tasa})")
        ...
        # 3. Validaciones específicas de Venezuela
        ...
        # 4. Procesamiento específico de Venezuela
        print(f"\n🔧 PROCESANDO DATOS...")
        print("-" * 30)
        ...
        if excel_processor.crear_archivo_consolidado(df_procesado, nombre_salida):
            print(f"\n✅ CONSOLIDADO VENEZUELA CON TIENDA COMPLETADO")
            print("=" * 70)
            ...
            print(f"   💵 Valor: {tasa_dolar:.4f} VES/USD")
        ...
        print(f"❌ Error procesando Venezuela: {e}")
        import traceback
        traceback.print_exc()
        return None
...
def generar_excel_venezuela_con_detalle(df_bosqueto_original: pd.DataFrame, 
    ...
    """
    print(f"📝 Generando Excel Venezuela con BOSQUETO + DETALLE CORREGIDO...")
    ...
    try:
        ...
        print(f"✅ Excel Venezuela generado: {temp_path}")
        return temp_path
...
def aplicar_estilos_excel_venezuela(archivo_path: str):
    """
    Aplicar estilos específicos de Venezuela al Excel generado
...
def obtener_info_venezuela():
    """Obtener información COMPLETA sobre el procesador de Venezuela"""
    return {
        'pais': 'Venezuela',
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        print("🧪 Uso: python venezuela.py archivo_pago.xlsx [archivo_absoluto.xlsx]")
